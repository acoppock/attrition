...
knitr::opts_chunk$set(
  eval = requireNamespace("vayr", quietly = TRUE) && requireNamespace("estimatr", quietly = TRUE)
)
```
...
The plot uses [vayr](https://cran.r-project.org/package=vayr) and the difference in means uses [estimatr](https://cran.r-project.org/package=estimatr). Both are suggested packages rather than required ones, so if either is missing the code below is shown but not run.
