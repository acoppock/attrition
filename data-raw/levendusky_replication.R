...
levendusky_replication <- read.csv("data-raw/levendusky_mturk_clean.csv", stringsAsFactors = FALSE)

levendusky_replication$Z_lev         <- factor(levendusky_replication$Z_lev, levels = c("Placebo", "Moderate", "Polarized"))
levendusky_replication$pid_3_recoded <- factor(levendusky_replication$pid_3_recoded, levels = c("Dem", "Ind", "Rep"))
levendusky_replication$Z_Levendusky  <- NULL   # duplicate of Z_lev in lower case
